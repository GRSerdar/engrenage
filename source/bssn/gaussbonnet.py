...
def compute_L_GB(bssn_vars, bssn_rhs, d1, d2, grid, background):
    ...
    #print('dKdt: ', np.min(dKdt))

    ### EXTRA STUFF TO BE ABLE TO TAKE THE LIE DERIVATIVE PART IN THE PERPENDICULAR PARTIAL DERIVATIVE.  

    # dKdt += np.einsum('xj,xj->x', background.inverse_scaling_vector * bssn_vars.shift_U, advec.K)
    ...
    D2_lapse = (em4phi*(np.einsum('xia, xai->x',bar_gamma_UU, d2_lapse)
                        ...
                        + 2* np.einsum('xai, xkl, xl, xk->x',bar_gamma_LL, bar_gamma_UU, d1_phi, d1_lapse)))
    ...
    ###################################
    '''# Debug stuff
    line1_term1 = -lapse*four_thirds*Trace_M*ilapse * dKdt
    line1_term2 = -lapse*four_thirds*Trace_M*(ilapse * D2_lapse )
    line1_term3 = -lapse*four_thirds*Trace_M*( - Asquared )
    line1_term4 = -lapse*four_thirds*Trace_M*( - one_third * K * K)

    line1 = 0* line1_term1 + line1_term2 + line1_term3 + line1_term4'''
    ###################################
    ...
    #print('dadt: ', np.min(dadt))

    ### Lie derivative shift term!!!!

    # dadt += get_tensor_advection(r, bssn_vars.a_LL, advec.a_LL, shift_U, d1.shift_U, background) 
    ...
    ##
    line2_beforeContraction = (ilapse[:, np.newaxis, np.newaxis] * e4phi[:, np.newaxis, np.newaxis]* dadt 
              + 2 * ilapse[:, np.newaxis, np.newaxis] * e4phi[:, np.newaxis, np.newaxis]* bar_A_LL_d_Shift_U_symmetric 
    ...
    line1_term2 = 8* lapse* (2*ilapse * np.einsum('xij, xij->', TraceFree_M_UU ,bar_A_LL_d_Shift_U_symmetric ) )
    line1_term3 = 8* lapse* (ilapse * np.einsum('xij, xij->x', TraceFree_M_UU, DkDl_lapse))
    line1_term4 = 0
    line2 =  line1_term1 + line1_term2 + line1_term3'''
    #________________________________________________________________________________________
    #Line 3
    ...
    line3_beforeContraction= (AkjAjl 
              ...
              + two_thirds * ilapse[:, np.newaxis, np.newaxis] * bar_div_shiftr[:, np.newaxis, np.newaxis] * bar_A_LL) #Are np.newaxis correct ? 
    ...
    #print(f"line1: {np.max(line1_term1):.3e}, line2: {np.max(line1_term2):.3e}, line3: {np.max(line1_term3):.3e}, line4: {np.max(line1_term4):.3e}, LGB: {np.max(L_GB):.3e}")
