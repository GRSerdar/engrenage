...
def compute_L_GB(bssn_vars, bssn_rhs, d1, d2, grid, background):
    ...
    # rescaled N_U
    r_N_U = background.scaling_vector * N_U
    ...
    # rescaled
    r_A_LL_d_shift_symmetric = background.inverse_scaling_matrix * A_LL_d_shift_symmetric
    ...
    Line2 = 8*(e4phi * np.einsum("xkl, xkl->x", r_TraceFree_M_UU, dadt_perp)
                + 2 * e4phi * np.einsum("xkl, xkl->x",r_TraceFree_M_UU, r_A_LL_d_shift_symmetric)
    ...
    Line3 = 8*(e4phi * bssn_vars.lapse * np.einsum("xkl, xkl->x",r_TraceFree_M_UU , r_AkjAjl)
               - two_thirds * e4phi * PreFac * np.einsum("xkl, xkl->x", r_TraceFree_M_UU, bssn_vars.a_LL))
    ...
    Line4 = (-4)*bssn_vars.lapse*(2*Product 
                                  ...
                                  - 0* four_thirds * one_third * D_L_K_D_U_K 
                                  - 2*N_squared)
    ...
    D_L_K_N_U_list.append(D_L_K_N_U)
    D_L_K_D_U_K_list.append(D_L_K_D_U_K)
