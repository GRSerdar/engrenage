...
def M_LL_AND_N_L(bssn_vars, d1, d2, grid, background):
    ################### BSSN variables ######################################
    r = grid.r
    N = grid.num_points
    ...
    return M_LL, Trace_M,  N_L, N_U, N_squared


#Coupling Functions

# You have to import the right scalar fields into this function
def Omega_L_AND_Omega_LL(bssn_vars, d1, d2, grid, background, u, v, d1_u, d1_v, d2_u):
    ...
    Omega_L = (- 4 * d2_Lambda_d2_u(lambda_GB)[:,np.newaxis] * v[:,np.newaxis]* d1_u
                   - 4 * d1_Lambda_d1_u(lambda_GB)[:,np.newaxis] * (d1_v + np.einsum("xjb, xib,xj->xi",bar_gamma_UU, bar_A_LL,d1_u))
                   + one_third * bssn_vars.K[:,np.newaxis] * d1_u)
    
    # D_i D_j u (scalar field)
    DiDj_u = (  d2_u 
                - np.einsum("xmij, xm->xij",bar_chris, d1_u)
                - 2 * np.einsum("xi, xj->xij",d1_u, d1.phi)
                - 2 * np.einsum("xj, xi->xij",d1_u,d1.phi)
                + 2 * np.einsum("xij, xml, xl, xm->xij",bar_gamma_LL, bar_gamma_UU, d1.phi, d1_u))
    ...
    Omega_LL = ( 4 * d1_Lambda_d1_u(lambda_GB)[:,np.newaxis, np.newaxis] * (DiDj_u + e4phi[:,np.newaxis,np.newaxis] * self.v[:,np.newaxis,np.newaxis] * (bar_A_LL + bar_gamma_LL * one_third * bssn_vars.K[:,np.newaxis,np.newaxis]))
                +4 * d2_Lambda_d2_u(lambda_GB)[:,np.newaxis, np.newaxis] * np.einsum('xi,xj->xij', d1_u, d1_u))
    
    Trace_Omega = get_trace(Omega_LL, gamma_UU)
    
    return Omega_L, Omega_LL, Trace_Omega


def F_AND_F_LL(bssn_vars, bssn_rhs, d1, d2, grid, background):
...
def compute_L_GB(bssn_vars, bssn_rhs, d1, d2, grid, background):
    ...
    # The barred L_GB does not contain any derivatives of K and Aij
    bar_L_GB = Line1 + Line2 + Line3

    # Now we define L_GB \perp
    # These containt the coefficients corresponding to dAdt and dKdt
    L_GB_K_perp = 8 * ilapse * e4phi * TraceFree_M_UU
    L_GB_A_perp = - ilapse * four_thirds * Trace_M
